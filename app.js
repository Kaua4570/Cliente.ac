...
const ConfigGeral = (() => {
  ...
  ], lojaLocalizacao: null, faixasEntrega: [] };
  const listeners = [];
  ...
  function getNextOrderNumber() {
    ...
    return fdb.runTransaction(async (tx) => {
      ...
      const atual = (doc.exists && doc.data().proximoNumeroPedido) || 1;
      tx.set(ref, { proximoNumeroPedido: atual + 1 }, { merge: true });
      return atual;
    }).catch(err => { toast('Erro ao gerar número do pedido: ' + err.message); return null; });
  }
...
function calcularTaxaEntrega(km) {
  ...
  return faixa ? faixa.preco : (faixas.length ? faixas[faixas.length - 1].preco : 0);
}
...
const PedidosCliente = (() => {
  ...
  function confirmarPagamento(id) {
    ...
    const venda = {
      ...
      localizacao: pedido.localizacao || null, taxaEntrega: taxa,
      statusEntrega: pedido.tipoEntrega === 'entrega' ? 'pendente' : null, entregadorId: null
    };
...
const PDV = (() => {
  ...
  async function finalize() {
    ...
    const venda = {
      ...
      localizacao: entregaLocalizacao, taxaEntrega: taxa,
...
const Entrega = (() => {
  function minhaRota() {
    ...
    if (!user) return [];
    return DB.getVendas().filter(v => v.entregadorId === user.id && v.statusEntrega && v.statusEntrega !== 'entregue');
  }
  ...
  function adicionarPedido() {
    ...
    if (!venda) { toast(`Pedido #${numero} não encontrado.`); return; }
    if (venda.tipoEntrega !== 'entrega') { toast(`Pedido #${numero} não é uma entrega.`); return; }
    if (venda.entregadorId && venda.entregadorId !== (Auth.getCurrentUser() || {}).id) {
      toast(`Pedido #${numero} já está com outro entregador.`); return;
    }
    const user = Auth.getCurrentUser();
  ...
  function marcarEntregue(id) {
    DB.updateVenda(id, { statusEntrega: 'entregue' });
  ...
  function render() {
    ...
    rota.forEach(v => {
      ...
      const waUrl = v.telefoneCliente ? `https://wa.me/${v.telefoneCliente.replace(/\D/g, '')}` : null;
      const itensResumo = v.itens.map(i => `${i.qtd}x ${i.nome}`).join(', ');
      const card = document.createElement('div');
      card.className = 'route-card';
      card.innerHTML = `
        <div class="route-head"><span class="route-num">Pedido #${v.numeroPedido}</span></div>
        <div class="route-nome">👤 ${v.nomeCliente || 'Cliente'}</div>
        <div class="route-end">📍 ${v.endereco || 'Endereço não informado'}</div>
        ...
        <button class="btn-entregue" data-entregue="${v.id}">✅ Entregue</button>`;
      container.appendChild(card);
    });
    container.querySelectorAll('[data-entregue]').forEach(b => b.addEventListener('click', () => marcarEntregue(b.dataset.entregue)));
  }
...
const Config = (() => {
  ...
  function addTaxa() {
    ...
    if (!ateKm || preco < 0 || isNaN(preco)) { toast('Preencha os km e o preço.'); return; }
